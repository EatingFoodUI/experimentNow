...
class Sampler(data.Dataset):
    def __getitem__(self, id):
        ...
        for tube_id in gt_sparse_tublet:
            ...
            if vname == 'MOT17-05-FRCNN':
                for ii, fid in enumerate(tublet_frame_ids):
                    ...
                    box[3] = np.clip(box[3], 0, h - 1)        centerFrame_index = np.zeros((self.max_objs,), dtype=np.int64) # 256 * 1
        ...
        all_center_index = np.zeros((self.max_objs, 1), dtype=np.int64) # 另一种格式
        ...
        for i in range(0, num_objs):
            ...
            # 绝对索引
            centerFrame_index[i] = OneCenterFrame_index

            # 看提取错index没有
            center_index[i] = index_all[i][OneCenterFrame_index * 2]

            tube_id_similarity[i] = np.full((1), 1.0 ,dtype=np.float32)
        ...
        gt = {'input': data, 'hm': hm, 'bezier_ctp': bezier_curves, 'wh': wh,
              ...
              'mask': mask,
              'centerFrame_index': centerFrame_index, # 轨迹中心帧是哪一帧
              # 物体（轨迹）在中心帧的的哪个位置 [256,1]
              'center_index': center_index,
              # 同一个轨迹同一个id比例 [256 1]
              'tube_id_similarity': tube_id_similarity, # 256 * 1
              # 每个轨迹在对应中心帧上的中心点索引 [256,1] problem
              'all_center_index': all_center_index # 256 * 1
              }
