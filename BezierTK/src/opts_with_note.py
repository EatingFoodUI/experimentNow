...
class opts(object):
    def __init__(self):
        ...
        self.parser.add_argument('--clip_len', default=1.0, type=float, help="sparse clip time interval")
        ...
        # 8 16
        self.parser.add_argument('--lr_step', type=str, default='8,16,24',
        ...
        # loss ratio settings
        self.parser.add_argument('--hm_weight', type=float, default=0.1,
        ...
        # 4
        self.parser.add_argument('--sliding_stride', type=int, default=4,
                                 help='sliding stride')
    ...
    # 通过数据集更新模型分支参数，方便后续模型的生成
    def update_dataset(self, opt, dataset):
        opt.nID = dataset.nID
        opt.num_classes = dataset.num_classes
        # 网络的分支的通道数
                           # hm通道数为1：中心帧特征图识别是否是这个物体
        opt.branch_info = {'hm': opt.num_classes,  # one for all / one for one
                           # bezier_ctp通道数为12：12个方向的偏移数值
                           'bezier_ctp': 12,
                           # wh通道数为2*当前序列使用的总帧数：每帧的检测框的长宽
