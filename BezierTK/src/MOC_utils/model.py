# import torchvision.models as models
import torch
import torch.nn as nn

import torch.utils.model_zoo as model_zoo
from network.moc_net import MOC_Net
from network.moc_det import MOC_Det, MOC_Backbone

# 创建整个网络的过程
def create_model(arch, branch_info, head_conv, K):
    # 模型的神经网络层数
    num_layers = int(arch[arch.find('_') + 1:]) if '_' in arch else 0
    # 使用的哪个神经网络
    arch = arch[:arch.find('_')] if '_' in arch else arch
    # head_conv 输出通道数(即类别)，k是在bezier曲线绘制时使用帧的间隔(一个序列帧的数量)
    # 创建整个网络bezier的架构
    model = MOC_Net(arch, num_layers, branch_info, head_conv, K)
    return model


def create_inference_model(arch, branch_info, head_conv, K):
    num_layers = int(arch[arch.find('_') + 1:]) if '_' in arch else 0
    arch = arch[:arch.find('_')] if '_' in arch else arch
    backbone = MOC_Backbone(arch, num_layers)
    branch = MOC_Det(backbone, branch_info, head_conv, K)
    return backbone, branch


def load_model(model, model_path, optimizer=None, lr=None):
    # 开始轮次
    start_epoch = 0
    # 导入训练好的模型
    checkpoint = torch.load(model_path, map_location=lambda storage, loc: storage)
    print('loaded {}, epoch {}'.format(model_path, checkpoint['epoch']))
    # 获取模型的参数
    state_dict_ = checkpoint['state_dict']
    state_dict = {}

    # convert data_parallal to model
    # 对模型的每层参数进行遍历
    # 修改参数的名称，copy到state_dict中
    for k in state_dict_:
        if k.startswith('module') and not k.startswith('module_list'):
            state_dict[k[7:]] = state_dict_[k]
        else:
            state_dict[k] = state_dict_[k]

    # 为state_dict添加model.state_dict的参数
    check_state_dict(model.state_dict(), state_dict)
    # 为模型导入训练好的，且格式正确的参数
    model.load_state_dict(state_dict, strict=False)

    # 确认训练的优化器参数是否正确
    # resume optimizer parameters
    if optimizer is not None:
        if 'optimizer' in checkpoint:
            # optimizer.param_groups = checkpoint['optimizer']['param_groups'].copy()
            optimizer.load_state_dict(checkpoint['optimizer'])
            start_epoch = checkpoint['epoch']
            for param_group in optimizer.param_groups:
                param_group['lr'] = lr
            print('Resumed optimizer with start lr', lr)
        else:
            print('No optimizer parameters in checkpoint.')

    if 'best' in checkpoint:
        best = checkpoint['best']
    else:
        best = 100
    if optimizer is not None:
        return model, optimizer, start_epoch, best
    else:
        return model


def load_inference_model(backbone, branch, model_path):
    checkpoint = torch.load(model_path, map_location=lambda storage, loc: storage)
    print('loaded {}, epoch {}'.format(model_path, checkpoint['epoch']))
    state_dict_ = checkpoint['state_dict']
    state_dict = {}

    # convert data_parallal to model
    for k in state_dict_:
        if k.startswith('module') and not k.startswith('module_list'):
            state_dict[k[7:]] = state_dict_[k]
        else:
            state_dict[k] = state_dict_[k]

    backbone.load_state_dict(state_dict, strict=False)

    branch.load_state_dict(state_dict, strict=False)

    return backbone, branch


def save_model(path, model, optimizer=None, epoch=0, best=100):
    if isinstance(model, torch.nn.DataParallel):
        state_dict = model.module.state_dict()
    else:
        state_dict = model.state_dict()
    data = {'epoch': epoch,
            'best': best,
            'state_dict': state_dict}
    if not (optimizer is None):
        data['optimizer'] = optimizer.state_dict()
    torch.save(data, path)

# 使用在ImageNet上面预训练的模型的参数S
def load_imagenet_pretrained_model(opt, model):
    model_urls = {
        'resnet18': 'https://download.pytorch.org/models/resnet18-5c106cde.pth',
        'resnet34': 'https://download.pytorch.org/models/resnet34-333f7ec4.pth',
        'resnet50': 'https://download.pytorch.org/models/resnet50-19c8e357.pth',
        'resnet101': 'https://download.pytorch.org/models/resnet101-5d3b4d8f.pth',
        'resnet152': 'https://download.pytorch.org/models/resnet152-b121ed2d.pth',
        'dla34': 'http://dl.yf.io/dla/models/imagenet/dla34-ba72cf86.pth'
    }
    arch = opt.arch
    if arch == 'dla_34':
        print('load imagenet pretrained dla_34')
        model_url = model_urls['dla34']
        model_weights = model_zoo.load_url(model_url)

    elif arch.startswith('resnet'):
        num_layers = int(arch[arch.find('_') + 1:]) if '_' in arch else 0
        assert num_layers in (18, 34, 50, 101, 152)
        arch = arch[:arch.find('_')] if '_' in arch else arch

        print('load imagenet pretrained ', arch)
        url = model_urls['resnet{}'.format(num_layers)]
        model_weights = model_zoo.load_url(url)
        print('=> loading pretrained model {}'.format(url))

    else:
        raise NotImplementedError
    new_state_dict = {}
    for key, value in model_weights.items():
        new_key = 'backbone.base.' + key
        new_state_dict[new_key] = value
    if opt.print_log:
        check_state_dict(model.state_dict(), new_state_dict)
    model.load_state_dict(new_state_dict, strict=False)
    if opt.ninput > 1:
        convert2flow(opt.ninput, model)

    return model

# 使用在coco上面预训练的模型的参数
def load_coco_pretrained_model(opt, model):
    if opt.arch == 'dla_34':
        print('load coco pretrained dla_34')
        model_path = '../experiment/modelzoo/coco_dla.pth'
    elif opt.arch == 'resnet_18':
        print('load coco pretrained resnet_18')
        model_path = '../experiment/modelzoo/coco_resdcn18.pth'
    elif opt.arch == 'resnet_101':
        print('load coco pretrained resnet_101')
        model_path = '../experiment/modelzoo/coco_resdcn101.pth'
    else:
        raise NotImplementedError
    checkpoint = torch.load(model_path, map_location=lambda storage, loc: storage)
    print('loaded {}, epoch {}'.format(model_path, checkpoint['epoch']))
    state_dict_ = checkpoint['state_dict']
    state_dict = {}

    # convert data_parallal to model
    for k in state_dict_:
        print(k)
        if k.startswith('module') and not k.startswith('module_list'):
            state_dict[k[7:]] = state_dict_[k]
        else:
            state_dict[k] = state_dict_[k]
    new_state_dict = {}
    for key, value in state_dict.items():
        if key.startswith('wh'):
            new_key = 'branch.' + key
            new_state_dict[new_key] = value
        else:
            new_key = 'backbone.' + key
            new_state_dict[new_key] = value

    print('load coco pretrained successfully')
    if opt.print_log:
        check_state_dict(model.state_dict(), new_state_dict)
        print('-' * 40)

    model.load_state_dict(new_state_dict, strict=False)

    return model

# 使用fairmot预训练模型的参数
def load_fairmot_pretrained_model(opt, model):
    if opt.arch == 'dla_34':
        print('load fairmot pretrained dla_34...')
        model_path = '../experiment/modelzoo/fairmot_dla34.pth'
        # model_path = '../experiment/modelzoo/mot17_without_ID.pth'
    else:
        raise NotImplementedError

    # 在每次训练后保存模型参数（权重）

    # 读取预训练模型参数
    checkpoint = torch.load(model_path, map_location=lambda storage, loc: storage)
    # 预训练模型信息
    print('loaded {}, epoch {}'.format(model_path, checkpoint['epoch']))
    # 模型参数
    state_dict_ = checkpoint['state_dict']
    state_dict = {}

    # convert data_parallal to model
    # 对模型的每层参数进行遍历
    # 修改参数的名称，copy到state_dict中
    for k in state_dict_:
        if k.startswith('module') and not k.startswith('module_list'):
            state_dict[k[7:]] = state_dict_[k]
        else:
            state_dict[k] = state_dict_[k]
    # 修改参数的名称，copy到new_state_dict中
    new_state_dict = {}
    for key, value in state_dict.items():
        if key.startswith('wh'):
            new_key = 'branch.' + key
            new_state_dict[new_key] = value
        elif key.startswith('hm'):
            new_key = 'branch.' + key
            new_state_dict[new_key] = value
        else:
            if 'fairmot_dla34' in model_path:
                new_key = 'backbone.' + key
            else:
                new_key = key
            new_state_dict[new_key] = value

    print('load coco pretrained successfully')
    if opt.print_log:
        # 检查要导入的模型参数
        check_state_dict(model.state_dict(), new_state_dict)
        print('-' * 40)

    # 为之前创建的模型添加上预训练模型的参数值
    model.load_state_dict(new_state_dict, strict=False)

    return model

# 修改要导入的参数的格式，使能正确导入到模型（load_dict）上
def check_state_dict(load_dict, new_dict):
    # check loaded parameters and created model parameters
    for k in new_dict:
        if k in load_dict:
            if new_dict[k].shape != load_dict[k].shape:
                print('Skip loading parameter {}, required shape {}, '
                      'loaded shape {}. '.format(k, load_dict[k].shape, new_dict[k].shape))
                new_dict[k] = load_dict[k]
        else:
            print('Drop parameter {}.'.format(k))
    for k in load_dict:
        if not (k in new_dict):
            print('No param {}.'.format(k))
            new_dict[k] = load_dict[k]
