...
from .losses import FocalLoss, RegL1Loss, RegL1Loss2, RegL1Loss3
...
class MOCTrainLoss(torch.nn.Module):
    ...
    def forward(self, output, batch):
        ...
        hm_loss = self.crit_hm2(output['hm'], batch['tube_id_similarity'],
                               batch['all_center_index'], batch['mask'])

        # mov_loss 增加 轨迹中心帧在那个位置
        # output['bezier_ctp']有两个部分
        mov_loss1 = self.crit_mov(output['bezier_ctp'][:,:18,:,:], batch['mask'],
                                 batch['index'], batch['bezier_ctp'])
        #
        mov_loss2 = self.crit_mov2(output['bezier_ctp'][:,18,:,:], batch['centerFrame_index'],
                                  batch['center_index'], batch['mask'])

        # 比例是多少 暂定1:1
        mov_loss = mov_loss1 + mov_loss2
