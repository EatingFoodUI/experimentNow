...
from MOC_utils.utils import ECC, readPickle, warp_bbox_iterate, warp_bbox_direct, warp_bbox_direct_v2
...
if __name__ == '__main__':
    ...
    warpmats_iterate = data['iterate_warp_mat'][vname]
    data = readPickle('../data/train/direct_warp_mat.pkl')
    warpmats_direct = data['direct_warp_mat'][vname]
    ...
    for start_frame_id in start_frame_range:
        ...
        for k in range(forward_frames):
            ...
            if base_frame_id == cur_frame_id:
                ...
                cv2.putText(base_img_bak, '{}'.format('C'), (iw - 150, ih - 100),
            ...
            if cur_frame_id in bbox:
                # print(bbox[cur_frame_id])
                align_bbox = warp_bbox_iterate(warpmats_iterate, base_frame_id, cur_frame_id, bbox[cur_frame_id])
                # print(align_bbox)
                # align_back_iterate = warp_bbox_iterate(warpmats_iterate, base_frame_id, cur_frame_id, align_bbox,
                #                                        decode=True)
                # print(tmp)
                # print('-'*80)
            else:
                # 下面的代码就不跑了，但是这样写不便于理解
                continue
            ...
            align_bbox1, cur_img = warp_bbox_direct_v2(warpmats_direct, base_frame_id, cur_frame_id,
                                                       bbox[cur_frame_id], cur_img)

            # align_bbox1 = warp_bbox_direct_v2(warpmats_direct, base_frame_id, cur_frame_id, bbox[cur_frame_id])
            # align_back_direct = warp_bbox_direct_v2(warpmats_direct, base_frame_id, cur_frame_id, align_bbox1,
            #                                         decode=True)

            # print('gap:')
            # print(cur_frame_id - base_frame_id)
            # print('iterate warp back:')
            # print(align_back_iterate - bbox[cur_frame_id])
            # print('direct warp back:')
            # print(align_back_direct - bbox[cur_frame_id])
            # print('-' * 80)
            # print('gap:')
            # print(cur_frame_id - base_frame_id)
            ...
            #             cv2.FONT_HERSHEY_COMPLEX, 0.5, (255, 255, 255), 1)
            #
            cv2.putText(bak_img, '{}'.format(cur_frame_id - base_frame_id), (iw - 200, ih - 200),
                        cv2.FONT_HERSHEY_COMPLEX, 2, (0, 255, 255), 2)
            base_bak_path = os.path.join(video_root, 'img2/{:0>6}.jpg'.format(base_frame_id))
            base_img_bak = cv2.imread(base_bak_path)
            ...
            cv2.imwrite(cur_bak_img_path, cur_img)
