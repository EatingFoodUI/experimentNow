...
def decode(heatmap, wh, bezier_ctp, max_objs, frame_stride, forward_frames):
    ...
    # byteTrack, high 0.8, conf_thres 0.5, low 0.1
    remain_inds = scores >= 0.5 # 下限索引
    inds_low = scores > 0.2
    inds_high = scores < 0.5
    # 二次association index
    inds_second = torch.logical_and(inds_high, inds_low)
    ...
    rctp = _tranpose_and_gather_feature(bezier_ctp[:,:12,:,:], index)
    ...
    # 添加mv
    expanded_center_ts = center_frames_eachFrames.clone().expand(batch, max_objs, 4)
    ctp = rctp.clone()
    ...
    # byteTrack
    detections_second = detections[inds_second].unsqueeze(0)
    ctp_second = ctp[inds_second].unsqueeze(0)

    return detections, ctp, detections_second, ctp_second
...
class JDETracker(object):
    ...
    def process(self, images, hm=None, frame_stride=1):
        ...
        if hm.ndim != 4:
            hm = hm.unsqueeze(1)
        # byteTrack
        detections, ctp, detections_second, ctp_second = decode(hm, wh, bezier_ctp, self.opt.max_objs,
    ...
    def update(self, images, meta, frame_id):
        ...
        # byteTrack
        detections, ctp, detections_second, ctp_second = self.process(images, hm, frame_stride=frame_stride[0])
        ...
        # not modify, bytetrack thresh is 0.7
