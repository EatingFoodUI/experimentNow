...
def decode(heatmap, wh, bezier_ctp, max_objs, frame_stride, forward_frames):
    ...
    # 用hm1的值sort hm
    # hm1 = _nms(center_frames_feature)
    # scores1, index1, xs1, ys1 = _topk(hm1, max_objs)
...
class JDETracker(object):
    ...
    def process(self, images, hm=None, frame_stride=1):
        ...
        with torch.no_grad():
            ...
            if hm is None:
                hm = rgb_output[0]['bezier_ctp'][:,12,:,:]
            wh = rgb_output[0]['wh']
            bezier_ctp = rgb_output[0]['bezier_ctp'][:,:12,:,:]
            # bezier_ctp = rgb_output[0]['bezier_ctp'][:,:-1,:,:]
            # centerFrame = bezier_ctp[:, 18, :, :].unsqueeze(0)
