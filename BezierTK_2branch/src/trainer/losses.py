...
class FocalLoss2(torch.nn.Module):
    ...
    # output['bezier_ctp'][:,12,:,:], batch['center_index'], batch['mask']
    def forward(self, out, center_index, mask):
...
# 修改的mov branch使用的loss（处理置信度）
class RegL1Loss4(torch.nn.Module):
    def __init__(self):
        super(RegL1Loss4, self).__init__()
