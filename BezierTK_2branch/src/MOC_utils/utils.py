...
# index：索引，物体中心点的一维化索引
def _gather_feature(feature, index, index_all=None):
    # dim = channel = 2*K 2×帧数
    # feature b, h*w , c
    # index  b, N --> b, N, c  批次 物体数量 通道数
    # index_all b, N, 2*K  (mov:no, wh,yes) 批次 物体数量 2×帧数

    if index_all is not None:
        index0 = index_all
    else:
        dim = feature.size(2) # 通道数
        index0 = index.unsqueeze(2).expand(index.size(0), index.size(1), dim) # 对于bezier_ctp：index (1,256); index0 (1,256,18)
        # index0 = index.expand(index.size(0), index.size(1), dim)
...
def _gather_feature2(feature, index, index_all=None):
    # dim = channel = 2*K 2×帧数
    # feature b, h*w , c
    # index  b, N --> b, N, c  批次 物体数量 通道数
    # index_all b, N, 2*K  (mov:no, wh,yes) 批次 物体数量 2×帧数
...
def _tranpose_and_gather_feature(feature, index, index_all=None):
    ...
    # b,h,w,c --> b,h*w,c view将h,w合成一维（类似于把图片变成序列表示）
    feature = feature.view(feature.size(0), -1, feature.size(3))
    # feature --> b, N, 2*K
    if index_all is None or index_all.sum() != 1:
        feature = _gather_feature(feature, index, index_all=index_all)
    # elif index_all.sum() == 1: # 修改的hm branch使用的loss
    else:
        feature = _gather_feature2(feature, index, index_all=index_all)
    return feature
