...
def random_affine(image, gt_sparse_tublet=None, degrees=(-10, 10), translate=(.1, .1), scale=(.9, 1.1), shear=(-2, 2),
    ...
    images_warp = image.copy()
    images_warp = cv2.warpPerspective(image, M, dsize=(width, height), flags=cv2.INTER_LINEAR,
                                             borderValue=borderValue)  # BGR order borderValue

    if gt_sparse_tublet is not None:
        # tublet in the clip
        for ind in range(len(gt_sparse_tublet)):
            targets = gt_sparse_tublet[ind]
            # n = targets[0]
            points = targets.copy()
            ...
            xy = (xy @ M.T)[:, :2].reshape(8)

            # create new boxes
            x = xy[[0, 2, 4, 6]]
            y = xy[[1, 3, 5, 7]]
            xy = np.array([x.min(), y.min(), x.max(), y.max()]).reshape(4)
            ...
            xy = np.array((x - w / 2, y - h / 2, x + w / 2, y + h / 2)).reshape(4).T
            ...
            # if not i:
            #     return image, gt_sparse_tublet, M

            targets = targets[i]
            targets = xy[i]
            gt_sparse_tublet[ind] = targets
...
def letterbox(image, height=608, width=1088,
    ...
    images_ret = np.zeros((height, width, 3), dtype=np.uint8)
    ...
    image_ret = cv2.copyMakeBorder(img, top, bottom, left, right, cv2.BORDER_CONSTANT,
                                           value=color)  # padded rectangular

    return image_ret, ratio, dw, dh
...
def _gather_feature_inference(feature, index, index_all=None):
    # dim = channel = 2*K
    # feature b, h*w , c
    # index  b, N --> b, N, c
    ...
    for i in range(index0.shape[0]):
        for j in range(index0.shape[1]):
            if index[0][i] == -1:
                feature[0][i][j] = 0
            feature[0][i][j] = feature[0][index[0][i]][j]
    # feature --> b, N, 2*K
    feature = feature[:, :index0.shape[0], :]
    return feature
...
def _tranpose_and_gather_feature_inference(feature, index, index_all=None):
    # b,c,h,w --> b,h,w,c
    feature = feature.permute(0, 2, 3, 1).contiguous()
    # b,h,w,c --> b,h*w,c
    feature = feature.view(feature.size(0), -1, feature.size(3))
    # feature --> b, N, 2*K
    feature = _gather_feature_inference(feature, index, index_all=index_all)
    return feature
