...
class MOT20(BaseDataset):
    ...
    def __init__(self, opt, mode, det='FRCNN'):
        ...
        # self._videos = ['MOT20-01', 'MOT20-03']
        # self._videos = ['MOT20-01', 'MOT20-02', 'MOT20-03', 'MOT20-05']
        self._indices = []
...
class eval_MOT20(MOT20):
    def __init__(self, opt, mode='val', vname='MOT20-01'):
        super(eval_MOT20, self).__init__(opt, mode=mode)
    ...
    def __getitem__(self, index):
        vname, frame_id = self._indices[index]
        ...
        image0 = cv2.imread(img_file)
        height, width, _ = image0.shape

        # images, ratio, padw, padh = self.pre_process_func(self.opt, image0)
        #
        # images_dict = {}
        # idx = 0
...
class eval_MOT20_public_det(MOT20):
    def __init__(self, opt, mode='val', vname='MOT20-01'):
        super(eval_MOT20_public_det, self).__init__(opt, mode=mode)
        self.input_h = self.height
        self.input_w = self.width
        self.output_h = self.input_h // self.opt.down_ratio
        self.output_w = self.input_w // self.opt.down_ratio
        ...
        if self.mode == 'val':
            mode = 'train'
            det_path = os.path.join(opt.root_dir, 'data', mode, vname, 'det/det.txt')
        elif self.mode == 'test':
            mode = 'test'
            det_path = os.path.join(opt.root_dir, 'data', mode, vname, 'det/det.txt')
