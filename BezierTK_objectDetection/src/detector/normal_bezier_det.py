...
class BezierDetector(object):
    ...
    def pre_process(self, images):
        ...
        data = np.empty((3, self.opt.input_h, self.opt.input_w), dtype=np.float32)
        data = np.transpose(images, (2, 0, 1))
        data = data / 255.

        return data

    def process(self, images, hm):
        with torch.no_grad():
            if self.rgb_model is not None:
                rgb_output = self.rgb_model(images)
                hm = rgb_output['hm'].sigmoid_()
                wh = rgb_output['wh']

        if hm.ndim != 4:
            hm = hm.unsqueeze(1)

        detections = decode(hm, wh, self.opt.max_objs,
                            forward_frames=self.opt.forward_frames)
    ...
    def post_process(self, detections,
        ...
        for i in range(detections.shape[0]):
            ...
            for j in range((cur_detections.shape[1] - 1) // 2):#2
    ...
    def run(self, data):
            ...
            hm = data['hm'].cuda()
            detections = self.process(images, hm)
